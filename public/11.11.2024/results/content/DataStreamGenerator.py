...
class DataStreamGenerator(object):
    ...
    def Plot_Simple(self, data, label , directory, type : str = "abrupt", variables : list = [], save_fig : bool = False):

        if save_fig:
            if not os.path.exists("./{}".format(directory)):
                os.mkdir("./{}".format(directory))
        size = 17 * np.ones(self.sample_count)
        ...
        plt.legend(loc=1)
        plt.title("t : INITIAL, {}".format(type))
        plt.xlabel('$x$')
        plt.ylabel('$y$')
        if save_fig:
            plt.savefig('./{}/INITIAL.png'.format(directory))
            plt.close()
        else:
            plt.pause(0.1)
            plt.show()
    ...
    def Plot(self, data, label, decision_boundary : str, directory, type : str = "abrupt", variables : list = [], save_fig : bool = False):
        ...
        for i in range(100):
            ...
            plt.ylabel('$y$')
            if save_fig:
                plt.savefig('./{}/T_{}.png'.format(directory, i+1))
                plt.close()
            plt.pause(0.1)
        if not save_fig:
            plt.show()
    ...
    def Linear_Abrupt(self, directory, plot : bool = False, save : bool = False):
        type = "linear_abrupt"
        ...
        if plot or save:
            DataStreamGenerator.Plot(self, data, label, "y-a-k*(x-b)=0", directory, type, [x_pass, y_pass], save)
        return data, label

    def Linear_Gradual_Rotation(self,directory, plot : bool = False, save : bool = False):
        ...
        data = DataStreamGenerator.Noise_And_Redunce(self, data, label, type)
        if plot or save:
            DataStreamGenerator.Plot(self, data, label, "y-a-k*(x-b)=0",directory, type, [x_spinaxis, y_spinaxis], save)
        return data, label

    def InitBatch(self,type ,directory, plot : bool = False, save : bool = False):
        type = DataStreamGenerator.Adjust_Type(self, type)
        data, label, l = DataStreamGenerator.Init_Sample_Linear(self)
        x_spinaxis = float(0)
        y_spinaxis = float(0)
        ...
        data = DataStreamGenerator.Noise_And_Redunce(self, data, label, type)
        if plot or save:
            DataStreamGenerator.Plot_Simple(self,data,label,directory, type,save_fig=save )
        return data, label


    def Linear_Sudden_Rotation(self,directory, plot : bool = False, save : bool = False):
    ...
    def Linear_Recurrent_Rotation(self,directory, plot : bool = False, save : bool = False):
